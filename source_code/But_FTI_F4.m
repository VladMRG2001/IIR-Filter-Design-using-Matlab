...
function [B,A,M] = But_FTI_F4(w_p,w_s,Delta_p,Delta_s,Ts)
...
%
	...
	M = ceil(log(((1+Delta_p+Delta_p)*(1+Delta_p-Delta_s))/((Delta_p*Delta_s^2)*(2+Delta_p))) / (2*log(w_s/w_p)));
    E1 = w_p/((Delta_p*(2+Delta_p))^(1/(2*M))) ; 	% Actualy, this is Omega_c 
